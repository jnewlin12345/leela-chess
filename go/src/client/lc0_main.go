...
func (c *CmdWrapper) launch(networkPath string, args []string, input bool) {
	...
	dir, _ := os.Getwd()
	c.Cmd = exec.Command(path.Join(dir, "lc0"), weights, "-training=true")
...
func train(networkPath string, count int, params []string,  sendGame func(path, pgn, version string) bool) {
...
func nextGame(httpClient *http.Client, count int) error {
	...
	if nextGame.Type == "train" {
		...
		train(networkPath, count, params,
			func(trainFile, pgn, version string) bool {
				// TODO: Keep track of pending uploads so that we don't end up with a
				// bunch of go routines piling up.
...
func main() {
	...
	firstGameResponse, err = fetchSettings(&client)
	if err != nil {
		log.Fatalf("Failed to fetch game settings: %v", err)
	}

	if firstGameResponse.Type != "train" {
		log.Fatalf("only training is currently supported")
	}
	networkPath, err := getNetwork(&httpClient, firstGameResp.Sha, true)
	if err != nil {
